...
class Class0DTests(unittest.TestCase):
    ...
    def test_unknown_molecule(self):
        ...
        system = Atoms(
            positions=[[0.0, 0.0, 0.0], [2.0, 0.0, 0.0]], symbols=["Au", "Ag"]
        )
        gap = 12
...
class Material2DTests(unittest.TestCase):
    ...
    def test_graphene_rectangular(self):
        system = Atoms(
            symbols=["C", "C", "C", "C"],
            cell=np.array(
                ([4.26, 0.0, 0.0], [0.0, 15, 0.0], [0.0, 0.0, 2.4595121467478055])
            ),
            positions=np.array(
                (
    ...
    def test_mos2_pristine_primitive(self):
        system = ase.build.mx2(
            formula="MoS2", kind="2H", a=3.18, thickness=3.19, size=(1, 1, 1), vacuum=8
        )
        system.set_pbc(True)
    ...
    def test_2d_split(self):
        ...
        system = Atoms(
            symbols=["H", "C"],
            cell=np.array(([2, 0.0, 0.0], [0.0, 2, 0.0], [0.0, 0.0, 15])),
            positions=np.array(
                (
                    [0, 0, 0],
                    [0, 0, 13.8],
    ...
    def test_boron_nitride(self):
        system = Atoms(
            ...
            positions=np.array(
                ([0, 0, 0], [-1.3823924100453746e-9, 1.3925688618963122, 0.0])
            ),
            pbc=True,
        )
        classifier = Classifier()
...
class Class3DTests(unittest.TestCase):
    ...
    def test_thin_sparse(self):
        ...
        system = Atoms(
            ...
            symbols=[5, 5, 51, 51],
            cell=np.array([[10.1, 0.0, 0.0], [0.0, 10.1, 0.0], [5.05, 5.05, 1.758333]]),
            pbc=[True, True, True],
        )
    ...
    def test_si(self):
        si = ase.lattice.cubic.Diamond(
            size=(1, 1, 1), symbol="Si", pbc=(1, 1, 1), latticeconstant=5.430710
        )
        classifier = Classifier()
    ...
    def test_graphite(self):
        ...
        system = ase.lattice.hexagonal.Graphite(
            size=(1, 1, 1), symbol="C", pbc=(1, 1, 1), latticeconstant=(2.461, 6.708)
        )
        classifier = Classifier()
        classification = classifier.classify(system)
        self.assertIsInstance(classification, Class3D)
    ...
    def test_too_sparse(self):
        ...
        system = ase.lattice.hexagonal.Graphite(
            size=(1, 1, 1), symbol="C", pbc=(1, 1, 1), latticeconstant=(2.461, 12)
        )
...
class SurfaceTests(unittest.TestCase):
    ...
    def test_surface_wrong_cm(self):
        ...
        # One adsorbate
        self.assertEqual(
            set(range(len(system))).difference(set([36])),
            set(classification.region.get_basis_indices()),
        )
    ...
    def test_ordered_adsorbates(self):
        ...
        # Adsorbates
        self.assertEqual(
            set(range(len(system))).difference(set(range(0, 13))),
            set(classification.region.get_basis_indices()),
        )
    ...
    def test_surface_with_one_basis_vector_as_span(self):
        ...
        # Adsorbates
        self.assertEqual(
            set(range(len(system))).difference(set(range(0, 6))),
            set(classification.region.get_basis_indices()),
        )
    ...
    def test_thin_complex_surface(self):
        ...
        # Adsorbates
        self.assertEqual(
            set(range(len(system))).difference(set([24, 25, 26])),
            set(classification.region.get_basis_indices()),
        )
    ...
    def test_bcc_substitution(self):
        ...
        # One defect
        self.assertEqual(
            set(range(len(system))).difference(set([42])),
            set(classification.region.get_basis_indices()),
        )
    ...
    def test_bcc_interstitional(self):
        ...
        # Pristine
        self.assertEqual(
            set(range(len(system))).difference(set([125])),
            set(classification.region.get_basis_indices()),
        )
    ...
    def test_surface_ads(self):
        ...
        # Adsorbates
        self.assertEqual(
            set(range(len(system))).difference(set([100, 101, 102])),
            set(classification.region.get_basis_indices()),
        )
    ...
    # This test case is disabled due to skipping the \omega_c criterion in v2.
    ...
    # This test is disabled in v2 as the chemical similarity check is completely
    # removed.
    ...
    #     # Adsorbates
    #     self.assertEqual(
    #         set(range(len(system))).difference(set(range(0, 18))),
    #         set(classification.region.get_basis_indices()),
    #     )
