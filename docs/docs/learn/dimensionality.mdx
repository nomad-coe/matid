...
If you wish to get the dimensionality of a `Cluster` object, then you can
directly use the `Cluster.get_dimensionality()` method, which is essentially a
shortcut to the `matid.geometry.get_dimensionality()` function.
